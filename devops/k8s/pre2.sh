...
nodeGroups:
  - name: ng-1
    ...
    ssh:
      allow: true
      publicKeyName: aws-automation-sandbox
    minSize: 2
    maxSize: 3
...
# create a cluster with above config
eksctl create cluster -f  ~/eks.yaml

## helm setup (uisng helm lower version as higher version does not contain tiller component)
...
master:
  ...
  ingress:
    enabled: true
    # For Kubernetes v1.14-, use 'extensions/v1beta1'
